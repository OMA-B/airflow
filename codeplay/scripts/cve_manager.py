...
def create_database(myuser, myhost, database, owner):
    ...
    try:
        ...
        try:
            con = connect(
                dbname="postgres", user=myuser, host=myhost, password=mypassword
            )
            dbname = database
...
def process_cves(directory, results, csv_file, import_db, myuser, myhost, database):
    if csv_file:
        ...
        file_cve_related_problems = open(
            results + "cve_related_problems.csv", "w", encoding="utf8"
        )
            ...
            try:
                writer_cvss.writerow(
                    [
                        ...
                        cves["impact"]["baseMetricV2"]["cvssV2"][
                            "confidentialityImpact"
                        ],
                        ...
                        str(cves["impact"]["baseMetricV2"]["severity"]),
                        description,
                        cves["publishedDate"],
                ...
                if cpe_list_length != 0:
                    for i in range(0, cpe_list_length):
                        if "children" in cves["configurations"]["nodes"][i]:
                            cpe_child_list_length = len(
                                cves["configurations"]["nodes"][i]["children"]
                            )
                            if cpe_child_list_length != 0:
                                for j in range(0, cpe_child_list_length):
                                    if (
                                        "cpe_match"
                                        in cves["configurations"]["nodes"][i][
                                            "children"
                                        ][j]
                                    ):
                                        cpes = cves["configurations"]["nodes"][i][
                                            "children"
                                        ][j]["cpe_match"]
                                        for cpe in cpes:
                                            if csv_file:
                            ...
                            if "cpe_match" in cves["configurations"]["nodes"][i]:
                                ...
                                cpe_inner_list_length = len(
                                    cves["configurations"]["nodes"]
                                )
    ...
    if import_db:
        ...
        try:
            ...
            if not os.path.exists(filename):
                raise RuntimeError(f"Unable to open file, path: {filename}")
            counter = 0
            with open(filename, "r", encoding="utf8") as f:
                ...
                for line in f:
                    ...
                    # Write the processed line to the StringIO buffer
            ...
            cur.copy_from(output, "cve_problem", sep="\t", columns=("cve", "problem"))
            conn.commit()
            f.close()
            ...
            cur.copy_from(
                output, "cpe", sep="\t", columns=("cve", "cpe23uri", "vulnerable")
            )
...
def execute_query(myuser, myhost, database, cve, score, date, csv_on, output_folder):
    ...
    try:
        ...
        try:
            ...
            if cve:
                cur.execute(
                    "SELECT cve, vector_string_3, base_score_3, base_severity_3, vector_string, base_score, severity, description, published_date, last_modified_date FROM cvss WHERE cve LIKE '%"
                    + cve
                    + "%'"
                )
                ...
                cur.execute(
                    "SELECT problem FROM cve_problem WHERE cve LIKE '%" + cve + "%'"
                )
                ...
                for i in selected_cve:
                    ...
                    if cwe.isdigit():
                        ...
                        if selected_cve2:
                            ...
                            print(i[0])
                cur.execute(
                    "SELECT cpe23uri FROM cpe WHERE cve LIKE '%"
                    + cve
                    + "%' AND vulnerable='True'"
                )
                selected_cve = cur.fetchall()
                ...
                if date:
                    cur.execute(
                        "SELECT cve, base_score_3,  vector_string_3, base_score, vector_string, published_date FROM cvss WHERE (base_score_3 >= "
                        + score
                        + "OR base_score >= "
                        + score
                        + ") AND (published_date >= '"
                    ...
                    selected_cves = cur.fetchall()
                    print(
                        "CVE \t\tCVSSv3.x Score CVSSv3.x Vector String \t\t\tCVSSv2 Score CVSSv2 Vector String"
                    )
                    for r in selected_cves:
            ...
            if csv_on:
                ...
                file_cve = open(
                    output_folder + "CVEs_score" + score + "_" + str(date) + ".csv", "w"
                )
...
def execute_query_cpe(
    myuser, myhost, database, cpe, score, date, csv_on, output_folder
):
    con = None
    mypassword = None
    if csv_on:
    ...
    try:
        ...
        try:
            ...
            if date:
                cur.execute(
                    "SELECT cpe23uri, cve, base_score_3, base_score, published_date FROM cvss_vs_cpes WHERE cpe23uri LIKE '%"
                    + cpe
                    + "%' AND (base_score_3 >= "
                    + score
                    + "OR base_score >= "
            ...
            if csv_on:
                if not os.path.exists(output_folder):
                    ...
                    print("Directory %s already exists" % output_folder)
                file_cpe = open(
                    output_folder + cpe + "_" + score + "_" + str(date) + ".csv", "w"
                )
                writer_cpe = csv.writer(file_cpe, delimiter=",")
                writer_cpe.writerow(
                    ["CPE", "CVE", "CVSSv3 Score", "CVSSv2 Score", "Published Date"]
                )
                for r in cpes:
    ...
    try:
        ...
        try:
            ...
            with open(filename, "r") as f:
                ...
                for row in reader:
                    writer_cwe.writerow(
                        [row[0], row[1], row[4], row[5], row[11], row[14], row[16]]
                    )
                cwe.seek(0)
                cwe.readline()
                cur.copy_expert(
